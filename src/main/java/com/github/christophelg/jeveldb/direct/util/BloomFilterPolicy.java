...
public class BloomFilterPolicy implements FilterPolicy {
  ...
  public byte[] AppendKeysToFilter(byte[][] keys, byte[] filter) {
    // Compute bloom filter size (in both bits and bytes)
    int bits = mBitPerKey * keys.length;
    // For a small # of keys, we can see a very high false positive rate.
    // Fix it by enforcing a minimum bloom filter length.
    bits = (bits < 64) ? bits = 64 : bits;

    int bytes = (bits + Byte.SIZE) / Byte.SIZE;
    bits = bytes * Byte.SIZE;

    int init_size = filter.length;
